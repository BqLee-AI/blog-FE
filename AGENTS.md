...
- 技术栈：Vite、React Router v7、Zustand、Tailwind CSS、Bun、ShadcnUI。
- 项目目标：围绕博客浏览、文章管理、评论交互、登录态和后台管理持续交付前端能力。
- 当前阶段：MVP 期，采用 Issue-Driven 开发；优先补齐认证、文章、评论、ShadcnUI 集成和真实 API 联调。
...
- 以验收标准为准：每个任务先确认对应 Issue、优先级和验收标准，再开始改动。
- 优先级排序：P0 安全与阻塞问题优先，其次 P1 核心链路，再处理 P2/P3 的体验和工程项。
...
- 若任务涉及 UI 交互、路由、表单或登录态，构建通过后仍需做一次手工路径检查。

## 4. 当前阶段任务重点

### Week 1-4 MVP 重点

- P0：认证链路、JWT 对接、文章模型/API、API 版本路由稳定。
- P1：axios 拦截器、Protected Route、真实登录/注册、文章列表与详情联调。
- P2：ShadcnUI 表单与基础组件、TinyMCE、reCAPTCHA、评论真实 API。
- P3：Docker、Nginx、基础 CI/CD。

### 分支与 Issue 规则

- 新任务优先从 `develop` 或当前集成分支切出功能分支。
- 分支命名建议：`feat/FE-xxx-*`、`feat/BE-xxx-*`、`fix/FE-xxx-*`、`fix/BE-xxx-*`、`chore/CI-xxx-*`。
- 每次提交尽量对应一个 Issue 或一个清晰的验收目标。
- 合并前先确认分支任务已结束，避免长期保留已完成的 feature 分支。
...
- 涉及登录、注册、路由守卫、文章 CRUD、评论交互时，除构建外要补充人工验证关键路径。
...
- 若引入 ShadcnUI、TinyMCE、reCAPTCHA、CI/CD 或新路由约定，README 也要同步更新。
...
- 登录态异常时，先看 `authStore`、`src/api/index.ts`、路由守卫和 `src/api/auth.ts`。
- ShadcnUI 样式异常时，先看主题变量、Tailwind 配置和组件导入路径。
